...
help(joi2ts, 'Create TypeScript type definitions from JOI schemas', {
  options: {
    ...
    label:
      "Use .label('InterfaceName') instead of .meta({className:'InterfaceName'}) for interface names. Default: false",
    ...
    flatten:
      'Will not output to subDirectories in output/interface directory. It will flatten the structure. Default: false',
    rootOnly:
      'Will only read the files in the root directory of the input/schema directory. Will not parse through sub-directories. Default: false',
